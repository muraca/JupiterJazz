...
public class RangeSlider extends JSlider {
    ...
    /**
     * Sets the lower and upper value in the range.
     */
    public void setValues(int lowerValue, int upperValue) {
        int newLowerValue = Math.min(Math.max(getMinimum(), lowerValue), getValue() + getExtent());
        int newExtent = Math.min(Math.max(0, upperValue - newLowerValue), getMaximum() - lowerValue);

        // Set new value and extent, and fire a single change event.
        getModel().setRangeProperties(newLowerValue, newExtent, getMinimum(),
                getMaximum(), getValueIsAdjusting());
    }
}
